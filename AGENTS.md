...
- **Modulação Adaptativa de Raciocínio & Comportamento:** Para tarefas mecânicas ou conceituais L0/L1 (dúvidas, docs, CSS, refatoração isolada), responda de forma direta e concisa. No CLI, use perfis com reasoning effort reduzido (`agy-fast` com `--effort low` para economizar 3.000 a 8.000 tokens de raciocínio). Na IDE, mantenha o seletor padrão em `Low` ou `Medium` para conversas gerais, reservando `High` exclusivamente para tarefas L2/L3.
...
- **Protocolo Anti-Flood & Zero-Tool para Consultas (L0):** Para perguntas conceituais, dúvidas teóricas, explicações ou bate-papo técnico, é ESTRITAMENTE PROIBIDO invocar ferramentas (`run_command`, `view_file`, `list_dir`, `grep_search`) — responda diretamente em texto puro, zerando o consumo de ferramentas. Ao executar ferramentas em tarefas práticas de código, nunca despeje saídas gigantes: use fatiamento cirúrgico em `view_file` (máx 50-80 linhas com `StartLine`/`EndLine`), flags silenciosas (`--silent`, `-q`) e `agy-sanitize` para truncar logs extensos.
