...
## Flow
Passo 0: Fast Context Bootstrap (`.agents/memory/PROJECT_MEMORY.md`) [com Auto-Onboarding se não inicializado/divergente] → **release-gatekeeper** (CI, Scans SAST/SCA, Assinatura Cosign/SBOM, Evidências) → **Passo Intermediário OBRIGATÓRIO (Hard-Enforcement Gate): Escrita Física em Disco no `.agents/memory/PROJECT_MEMORY.md` (tags & memory lint) pelo `archivist`** → **shipper** (**Zero-Downtime Deploy: Blue/Green ou Canary** → Startup/Readiness Probes → Telemetria RED pós-deploy → Monitoramento de SLOs)
